...
const URL =
  'https://us-central1-fiery-marking-375416.cloudfunctions.net/predict';
  ...
  return (
    <View style={[backgroundStyle, styles.outer]}>
      ...
      <TouchableOpacity onPress={clearOutput} style={styles.clearStyle}>
        ...
        <Text style={{fontSize:25, letterSpacing:1, color:'white'}}><Image source={{uri: 'clean'}} style={styles.clearImage} />{'clear'}</Text>
      </TouchableOpacity>
      {(image?.length && (
      ...
      {(result && label && (
        ...
        (image && <Text style={styles.emptyText}>{label}</Text>) || (
          <Text style={styles.emptyText}>
            Select a picture of a potato plant leaf.
          </Text>
        )}
...
const styles = StyleSheet.create({
  ...
  resultText: {fontSize: 16, ...fonts.Bold},
